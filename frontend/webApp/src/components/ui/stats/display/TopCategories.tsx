...
const TopCategories = ({periodLabel, categories}: TopCategoriesProps) => {
    return (
                    ...
                    <article className="rounded-2xl bg-surface-base p-3 ring-1 ring-border-subtle sm:p-3.5" key={category.id}>
                        ...
                        <div className="mt-2.5 h-2 rounded-full bg-border-strong sm:mt-3">
                            <div
                                ...
                                style={{backgroundColor: category.color ?? "#CBD5E1", width: `${Math.min(category.share * 2.2, 100)}%`}}
                            />
                        </div>
                        <p className="mt-1.5 text-xs font-semibold sm:mt-2 sm:text-sm" style={{color: category.color ?? "#64748B"}}>
