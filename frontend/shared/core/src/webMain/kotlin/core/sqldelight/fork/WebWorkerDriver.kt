...
class WebWorkerDriver(private val worker: Worker) : SqlDriver {
    ...
    private val tabId = Uuid.generateV7().toString()

    private val listeners = mutableMapOf<String, MutableSet<Query.Listener>>()
    private var messageCounter = 0
    private var transaction: Transacter.Transaction? = null
    private val wrapper = WorkerWrapper(worker) { data ->
        val tables = data.tables?.unsafeCast<Array<String>>() ?: emptyArray()

        if (tables.isEmpty()) {
            notifyListeners(*listeners.keys.toTypedArray())
        } else {
            notifyListeners(queryKeys = tables)
        }
    }
