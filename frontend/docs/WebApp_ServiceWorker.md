# DEPRECATED. Взаимодействие WebApp и Service Worker

> [!IMPORTANT]
> Данный документ устарел в связи с тем, что теперь для хранения в бд используется OPFS из-за чего фоновая 
> синхронизация больше недоступна → ServiceWorker был переписан. (вырезан k2ts-service, убрана фоновая синхронизация)
...
Используется библиотека sql-wasm (+idb*). Бинарные файлы (.js и .wasm) лежат в папке /db.
